...
function renderBidTotals(total) {
    ...
    if (bidTotalPriceEl) bidTotalPriceEl.textContent = `$${safeTotal.toFixed(2)}`;
    if (bidSellerRevenueEl) bidSellerRevenueEl.textContent = `$${(safeTotal * 0.95).toFixed(2)}`;
    if (bidServiceFeeEl) bidServiceFeeEl.textContent = `$${(safeTotal * 0.05).toFixed(2)}`;
}
...
function recomputeBidTotals() {
    ...
    const rawPct = (bidPartialPercentInput ? bidPartialPercentInput.value : "").trim();
...
if (modalFixedBlock) modalFixedBlock.style.display = isBid ? "none" : "block";
...
function renderFixedTotals(total) {
  ...
  if (modalSellerRevenue) modalSellerRevenue.textContent = `$${sellerRevenue.toFixed(2)}`;
  if (modalServiceFee) modalServiceFee.textContent = `$${serviceFee.toFixed(2)}`;
}
...
function computeFixedBooking(space) {
  ...
  if (base == null || base <= 0) return { ok: false, error: "Invalid fixed price." };
  ...
  const rawPct = (partialPercentInput ? partialPercentInput.value : "").trim();
...
function computeBidBooking() {
  ...
  if (!rawBid || bidValue == null || bidValue <= 0) {
    return { ok: false, error: "Please enter a valid bid amount before booking." };
  }
  ...
  const isPartialOpen = bidPartialBox && bidPartialBox.style.display !== "none";
  const rawPct = (bidPartialPercentInput ? bidPartialPercentInput.value : "").trim();
...
if (bookBtn && !bookBtn.__wired) {
  ...
  bookBtn.addEventListener("click", async function (e) {
    ...
    const refId =
      activeSpace.RefID || activeSpace.RefId || activeSpace.refId || activeSpace.refID;
    ...
    const pricing = isBid ? computeBidBooking() : computeFixedBooking(activeSpace);
    if (!pricing.ok) {
